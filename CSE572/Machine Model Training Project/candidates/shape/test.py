...
def extract_base_features(samples):
    ...
    return np.asarray(rows, dtype=float).reshape(-1, len(BASE_FEATURE_NAMES))


FEATURE_NAMES = BASE_FEATURE_NAMES + ['max_15min_rise', 'min_15min_change', 'max_30min_rise', 'min_30min_change', 'max_60min_rise', 'min_60min_change', 'early_slope', 'late_slope', 'longest_rise_fraction', 'longest_fall_fraction', 'lag1_correlation', 'rise_after_trough_fraction', 'peak_above_end', 'early_relative_rise']


def extract_features(samples):
    """Append 14 timing and excursion features to the original 24 features."""
    base=extract_base_features(samples)
    extra=[]
    for row in samples:
